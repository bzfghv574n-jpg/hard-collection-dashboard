...
const emptyForm = { name: '', car_brand: '', car_model: '', engine_volume: '', fuel_type: 'бензин', fuel_consumption_city: '', fuel_consumption_highway: '', color: '#3B82F6' };

export default function AdminPanel() {
  const [tab, setTab] = useState('crews');
  const [crews, setCrews] = useState([]);
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState(null); // null = создание, id = редактирование
  ...
  useEffect(() => {
    ...
    axios.get(`${API}/fuel-prices`).then(r => {
      ...
      setFuelForm({ бензин: prices['бензин'] || 245, дизель: prices['дизель'] || 260, газ: prices['газ'] || 95 });
    }).catch(() => {});
  }, []);
  ...
  const openEdit = (crew) => {
    ...
    setForm({
      name: crew.name,
      car_brand: crew.car_brand,
      car_model: crew.car_model,
      engine_volume: String(crew.engine_volume),
      fuel_type: crew.fuel_type,
      fuel_consumption_city: String(crew.fuel_consumption_city),
      fuel_consumption_highway: String(crew.fuel_consumption_highway),
  ...
  const submit = async () => {
    ...
    if (editingId) {
      // Редактирование
      try {
        await axios.put(`${API}/crews/${editingId}`, {
          ...form,
        ...
        setCrews(crews.map(c => c.id === editingId ? {
          ...c, ...form,
          engine_volume: parseFloat(form.engine_volume),
          fuel_consumption_city: parseFloat(form.fuel_consumption_city),
          fuel_consumption_highway: parseFloat(form.fuel_consumption_highway),
    ...
    <div style={{ padding: 20, maxWidth: 860, margin: '0 auto', overflow: 'auto', height: 'calc(100vh - 52px)' }}>
      ...
      {tab === 'crews' && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <button onClick={openCreate} style={{ padding: '8px 16px', borderRadius: 8, background: '#3B82F6', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>+ Новый экипаж</button>
          </div>
          ...
          {showForm && (
            <div style={{ background: '#0E1117', border: '1px solid #3B82F655', borderRadius: 12, padding: 16, marginBottom: 16 }}>
              <div style={{ color: '#F1F5F9', fontWeight: 700, fontSize: 14, marginBottom: 14 }}>
                {editingId ? '✏️ Редактировать экипаж' : 'Создать экипаж'}
              </div>
              ...
              {/* Сотрудники — только при создании */}
              {!editingId && (
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                <button onClick={submit} style={{ flex: 1, padding: '10px', borderRadius: 8, background: '#3B82F6', border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                  {editingId ? 'Сохранить' : 'Создать'}
                </button>
                <button onClick={() => { setShowForm(false); setEditingId(null); }} style={{ padding: '10px 16px', borderRadius: 8, background: 'transparent', border: '1px solid #2A2F42', color: '#475569', fontSize: 13, cursor: 'pointer' }}>Отмена</button>
              </div>
            </div>
          )}
            ...
            {crews.map(c => (
              <div key={c.id} style={{ background: '#0E1117', border: '1px solid #2A2F42', borderRadius: 10, padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <button onClick={() => openEdit(c)} style={{ padding: '6px 12px', borderRadius: 7, background: 'transparent', border: '1px solid #3B82F644', color: '#3B82F6', fontSize: 12, cursor: 'pointer', flexShrink: 0 }}>✏️</button>
                  <button onClick={() => deleteCrew(c.id, c.name)} style={{ padding: '6px 12px', borderRadius: 7, background: 'transparent', border: '1px solid #EF444444', color: '#EF4444', fontSize: 12, cursor: 'pointer', flexShrink: 0 }}>🗑</button>
                </div>
                          ...
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#151820', borderRadius: 8, padding: '7px 10px', border: '1px solid #2A2F42' }}>
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 3 }}>
                                <span style={{ color: '#22C55E', fontSize: 11, fontFamily: 'monospace' }}>Логин: {cm.employees?.login}</span>
                                <CopyBtn text={cm.employees?.login || ''} />
                              </div>
                              {newPass && (
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 3 }}>
                                  <span style={{ color: '#F59E0B', fontSize: 11, fontFamily: 'monospace' }}>Новый пароль: {newPass}</span>
                                  <CopyBtn text={newPass} />
                                </div>
                              )}
                            </div>
                            <button onClick={() => resetPassword(empId)} style={{ padding: '5px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer', background: 'transparent', border: '1px solid #F59E0B44', color: '#F59E0B', flexShrink: 0 }}>🔑 Сбросить пароль</button>
                          </div>
                        );
                      })}
                    </div>
      ...
      {tab === 'fuel' && (
        <div style={{ background: '#0E1117', border: '1px solid #2A2F42', borderRadius: 12, padding: 20, maxWidth: 500 }}>
          ...
          <div style={{ color: '#475569', fontSize: 11, marginBottom: 20 }}>Используются для расчёта стоимости в отчётах.</div>
          {FUEL_TYPES.map(ft => (
            <div key={ft} style={{ marginBottom: 14 }}>
              <div style={{ color: '#475569', fontSize: 10, fontWeight: 600, letterSpacing: '0.06em', marginBottom: 6 }}>{FUEL_LABELS[ft].toUpperCase()} (₸/литр)</div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                {fuelPrices[ft] && <span style={{ color: '#475569', fontSize: 11 }}>Текущая: {fuelPrices[ft]} ₸</span>}
              </div>
            </div>
          ))}
